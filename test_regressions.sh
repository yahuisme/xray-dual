...
expect_false 'reject leading-zero port' is_valid_port 0443
expect_false 'reject leading-zero port 2' is_valid_port '00022'
...
if grep -Fq 'readonly SCRIPT_VERSION="v26.09.02"' "$script"; then
...
if grep -Fq 'Xray 未安装，无需卸载。' "$script" &&
   grep -Fq '/etc/systemd/system/xray.service' "$script"; then
    pass 'uninstall shows not-installed notice'
else
    fail 'uninstall shows not-installed notice'
fi

if grep -Fq 'existing_ss_inbound=$(get_managed_inbound shadowsocks)' "$script" &&
   grep -Fq 'existing_vless_inbound=$(get_managed_inbound vless)' "$script"; then
    pass 'single-protocol installs merge existing managed inbound'
else
    fail 'single-protocol installs merge existing managed inbound'
fi

if grep -Fq '10#$octet' "$script" && grep -Fq 'if ! ip=$(get_public_ip); then' "$script"; then
    pass 'public-ip octet validation and failure handling'
else
    fail 'public-ip octet validation and failure handling'
fi
