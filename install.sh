...
add_ss_to_vless() {
    ...
    if [[ -z "$ss_password" ]]; then
        ss_password=$(generate_ss_key); info "已为您生成随机密钥: ${cyan}${ss_password}${none}";
    fi
...
add_vless_to_ss() {
    ...
    if [[ "$ss_port" == "8388" ]]; then
        default_vless_port=443
    else
        default_vless_port=$((ss_port - 1))
    fi
    ...
    read -p "$(echo -e "请输入UUID (留空将默认生成随机UUID): ")" vless_uuid
    if [[ -z "$vless_uuid" ]]; then
        vless_uuid=$(cat /proc/sys/kernel/random/uuid); info "已为您生成随机UUID: ${cyan}${vless_uuid}${none}";
    fi
...
install_vless_only() {
    ...
    if [[ -z "$uuid" ]]; then
        uuid=$(cat /proc/sys/kernel/random/uuid); info "已为您生成随机UUID: ${cyan}${uuid}${none}";
    fi
    while true; do
...
install_ss_only() {
    ...
    if [[ -z "$password" ]]; then
        password=$(generate_ss_key); info "已为您生成随机密钥: ${cyan}${password}${none}";
    fi
...
install_dual() {
    ...
    read -p "$(echo -e "请输入 Shadowsocks 密钥 (留空将自动生成): ")" ss_password
    if [[ -z "$ss_password" ]]; then
        ss_password=$(generate_ss_key); info "已为您生成 Shadowsocks 随机密钥: ${cyan}${ss_password}${none}";
    fi
...
modify_ss_config() {
    ...
    if [[ -z "$password_input" ]]; then
        new_password=$current_password; info "密钥未修改。";
    else
        new_password=$password_input; info "密钥已更新。";
    fi
...
main_menu() {
    while true; do
        clear; echo -e "$cyan Xray 多功能管理脚本$none";
        echo "---------------------------------------------"
