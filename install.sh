...
pre_check() {
    ...
    for cmd in jq curl openssl systemctl; do
        if ! command -v $cmd &>/dev/null; then
            ...
            check_apt_source
            (apt-get update && apt-get install -y $cmd) &> /dev/null || { error "安装 $cmd 失败，请手动安装。"; exit 1; }
        fi
    done
}
...
write_config() {
    ...
    if ! $XRAY_BINARY_PATH validate -c "$XRAY_CONFIG_PATH" &>/dev/null; then
...
install_menu() {
    ...
    if [[ -n "$vless_exists" && -n "$ss_exists" ]]; then
        ...
        case $choice in
            ...
            0) return ;;
            *) error "无效选项，请选择 0-2。" && read -p "按 Enter 键继续..." ;;
        esac
...
clean_install_menu() {
    ...
    case $choice in
        ...
        3) install_dual ;;
        0) return ;;
        *) error "无效选项，请选择 0-3。" && read -p "按 Enter 键继续..." ;;
    esac
}
...
update_xray() {
    ...
    local latest_version=$(curl -s --connect-timeout 5 https://api.github.com/repos/XTLS/Xray-core/releases/latest | jq -r '.tag_name' | sed 's/v//')
    if [[ -z "$latest_version" ]]; then error "获取最新版本号失败，请检查网络连接。" && return; fi
    ...
    info "发现新版本，开始更新..."; run_core_install && restart_xray && success "Xray 更新成功！"
}
...
modify_config_menu() {
    ...
    if [[ -n "$vless_exists" && -n "$ss_exists" ]]; then
        clear; echo "---------------------------------------------"; echo -e "$cyan  请选择要修改的配置$none"; echo "---------------------------------------------";
        printf "  ${green}%-2s${none} %-35s\n" "1." "VLESS-Reality"; printf "  ${cyan}%-2s${none} %-35s\n" "2." "Shadowsocks-2022";
        echo "---------------------------------------------"; printf "  ${green}%-2s${none} %-35s\n" "0." "返回主菜单"; echo "---------------------------------------------";
        read -p "请输入选项 [0-2]: " choice
        case $choice in
...
view_all_info() {
    ...
    if [[ -z "$ip" ]]; then
        ip=$(curl -4s --connect-timeout 5 https://www.cloudflare.com/cdn-cgi/trace | grep -oP 'ip=\K.*$' || curl -6s --connect-timeout 5 https://www.cloudflare.com/cdn-cgi/trace | grep -oP 'ip=\K.*$')
        if [[ -z "$ip" ]]; then
    ...
    if [[ -n "$vless_inbound" ]]; then
        ...
        if [[ -z "$public_key" ]]; then error "VLESS配置不完整，请重新安装。" && return; fi
    ...
    if [ ${#links_array[@]} -gt 0 ]; then
        ...
        chmod 600 ~/xray_subscription_info.txt
        echo "----------------------------------------------------------------"; echo -e "$green 所有链接已汇总保存到 ~/xray_subscription_info.txt $none";
...
main_menu() {
    while true; do
        ...
        case $choice in
            ...
            *) error "无效选项，请选择 0-7。" && read -p "按 Enter 键继续..." ;;
        esac
        read -p "按 Enter 键返回主菜单..."
    done
}
